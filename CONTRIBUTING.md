...
`firstpass` is an ESM-only Node project (Node 20+) using the pnpm version pinned in `packageManager`.
...
The global `firstpass` binary is a built snapshot, so reinstall it (`npm pack` then `npm install -g ./firstpass-<version>.tgz`) only when you want to exercise the packaged install.
...
- `pnpm run package:check` builds the package, installs it globally into a temporary prefix, and verifies the installed `firstpass` binary actually runs.
